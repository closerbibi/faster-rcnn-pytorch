...
def test_net(name, net, imdb, max_per_image=300, thresh=0.05, vis=False):
    ...
    # Hao python evaluating
    imdb.evaluate_detections(all_boxes, output_dir, save_name, result_name)

    # my matlab evaluating
    '''
    _iter = trained_model.split('_')[-1].split('.')[0]
    result_type = imdb_name.split('_')[-1]
    result_name = 'hha_%s'%_iter
    save_name = name.split('/')[0]
    #pdb.set_trace()
    imdb._write_inria_results_file(all_boxes, save_name, result_name)
    cmd = "cd ./evaluation && matlab -nodesktop -r \"runall('{}', '{}', '{}'); exit;\" " \
                                            .format(result_type, save_name, result_name)
    #pdb.set_trace()
    os.system(cmd)
    #imdb.evaluate_detections(all_boxes, result_name, output_dir)
    #"matlab.exe -nosplash -nodesktop -wait -r \"addpath('c:\\some\\folder'); someFunction(); quit();\""
    '''
